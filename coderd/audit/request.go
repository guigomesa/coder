...
type Auditor[T Auditable] interface {
	InitRequest(w http.ResponseWriter, p *RequstParams) (*Request[T], func())
}
...
func InitRequest[T Auditable](w http.ResponseWriter, p *RequstParams) (*Request[T], func()) {
	...
	return req, func() {
		...
		_, err := p.DB.InsertAuditLog(ctx, database.InsertAuditLogParams{StatusCode: int32(code)})
		if err != nil {
			p.Log.Error(ctx, "insert audit log", slog.Error(err))
		}
	}
}
